...
import { Sofa, Users, DollarSign, Clock } from "lucide-react"
import { useEffect, useState } from "react"
import { createClient } from "@/lib/supabase/client"

interface MebelStats {
  jamiMijozlar: number
...
export function MebelStats() {
  const [stats, setStats] = useState<MebelStats>({
    jamiMijozlar: 0,
    doimiyMijozlar: 0,
    jamiSumma: 0,
    jamiQarz: 0,
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      try {
        const { data, error } = await supabase.from("mebel").select("mijoz_turi, qanchaga_kelishildi, qancha_qoldi")
